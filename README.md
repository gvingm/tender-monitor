...
Триггер: карточка со статусом **«На рассмотрении»** или воронкой **«интересные»**
(общая очередь, дедуп по record_id). Сервис скачивает файлы тендера в карточку:

1. Поиск в Bitable: `Статус = "На рассмотрении"` AND `ФайлыЗагружены = false`,
   плюс `Воронка = "интересные"` AND `ФайлыЗагружены = false`
   (если поле `ФайлыЗагружены` отсутствует — fallback: тот же фильтр + пустое поле `Файлы`).
...
5. Семантика **«все или ничего»**: карточка обновляется только когда ВСЕ файлы скачаны
   и загружены. При частичных ошибках запись не трогаем — повтор в следующем проходе.
6. Для карточек воронки «интересные» после успешной загрузки всех файлов (в т.ч. когда
   вложений нет) поле «Воронка» в той же PUT-записи переводится в **«Документы скачал»**.
   Для «На рассмотрении» воронка не меняется.
7. Каждый файл логируется (имя, размер, результат). Временные файлы удаляются после загрузки.
...
## Обновление 02.09.2026 — воронки, метки, анализ ТЗ

### Воронки
- Поле Bitable «Воронка» (Text, автосоздание).
- Ежедневный сбор 08:00 МСК: новые тендеры → воронка «новые от ДД.ММ.ГГГГ».
- Метка Tenderplan «Интересно» → воронка «интересные» (опрос меток каждые 5 мин:
  v1 `/api/tenders/getlist` → marks (ObjectId) → `/api/marks/get?id=` → имя, фильтр (?i)интерес).

### Файловый пайплайн (расширен)
Обрабатывает и карточки «На рассмотрении», и воронку «интересные» (общая очередь, дедуп по record_id).
Семантика «все или ничего»: частичная загрузка не фиксируется — будет повтор в следующем проходе.
Когда все документы интересной карточки загружены (или вложений нет), воронка переводится
в «Документы скачал». Карточки, у которых файлы были загружены старой версией кода без
перевода воронки, догоняются автоматически при следующем проходе.

### Первичный анализ ТЗ (только дноуглубительные из «интересные»)
Дноуглубительный = (?i)(дноуглуб|расчистка русл|углубление дна/русла/акватории).
После скачивания файлов выбираются кандидаты ТЗ (имя с «ТЗ/техническ/задание», иначе ≤3 первых, >50 МБ пропуск),
файл уходит в Kimi Files API (`/v1/files`, purpose=file-extract) → текст → chat (kimi-k2.6) →
JSON {obem, otval, tehnika, cena_m3, komment} по 4 критериям:
объём грунта выемки/отсыпки; расстояние до отвала; необходимая техника; стоимость 1 м³
(если цены нет в ТЗ — оценка НМЦ/объём с пометкой «оценка»).
Результат → поля «Объём грунта», «Отвал: расстояние», «Техника», «Стоимость 1 м³», «Анализ ТЗ».
Повторно не анализирует (проверка заполненности + троттлинг 1ч).

### Endpoints (добавлено)
- `GET /report` — JSON последнего прогона: счётчики, интересные тендеры со статусами, ошибки.
